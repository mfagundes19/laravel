            ...
            {!! Form::open(['url' => url($route->module.'/'.$route->action),'id' => 'form-'.$route->module,'method' => 'POST','data-action' => url($route->module.'/save'),'data-refresh' => true]) !!}  
                ...
                <div class="row col-lg-12">
                    ...
                    <div class="col-lg-12">
                        <div class="form-group">
                            {!! Form::label('Família') !!}:
                            {!! Form::select('category_id', \App\Models\ProductCategory::where([['category_id',NULL],['active', true]])->pluck('name','id'), $category_id ?? '', ['placeholder' => 'Selecione', 'id' => 'category_id','class' => 'form-control','data-rule-required' => 'true','data-msg-required' => '* Campo Obrigatório']) !!}
                        </div>
                    </div>
                </div>
