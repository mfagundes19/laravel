...
class PartnerTypeController extends Controller
{
    ...
    {
        $PartnerTypeRepository = new PartnerTypeRepository();
        $parameters = array();
        ...
        $parameters['list'] = $PartnerTypeRepository->findBy($parameters, ['id ASC'], 20, 10);
     ...
     *
     * @param  \Illuminate\Http\Request  $request
     * @param  \Integer $partner_type_id
     * @return \Illuminate\Http\Response
     */
    public function edit(Request $request, $partner_type_id = null)
    {        
        if(Auth::user()->hasPermission($this->route->module,$this->route->action)) {
            $parameters = array();
            $parameters = $request->except('_token');
            $parameters['partner_type_id'] = (isset($parameters['partner_type_id'])) ? ($parameters['partner_type_id']) : $partner_type_id; 
            $parameters['name'] = (isset($parameters['name'])) ? ($parameters['name']) : ""; 
            $parameters['route'] = $this->route;
            if(isset($partner_type_id) && !empty($partner_type_id)) {                               
                $PartnerTypeRepository = new PartnerTypeRepository();
                $PartnerType = $PartnerTypeRepository->findById($partner_type_id);
                foreach($PartnerType->toArray() as $key => $value) {
     ...
     * @return \Illuminate\Http\Response
     */
    public function delete(Request $request, $partner_type_id)
    {        
        if(Auth::user()->hasPermission($this->route->module, $this->route->action)) {  
            $PartnerTypeRepository = new PartnerTypeRepository();
            if(isset($partner_type_id) && !empty($partner_type_id)) {
                $PartnerType = $PartnerTypeRepository->findById($partner_type_id);
                $PartnerType->active = false;
    ...
    public function save(Request $request)
    {
        ...
        switch($procedure) {
            case 'create':   //Create
                ...
                $PartnerTypeRepository = new PartnerTypeRepository();
                $PartnerType->name = (!empty($name)) ? $name : "";
