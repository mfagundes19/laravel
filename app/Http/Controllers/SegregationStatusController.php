...
use App\Models\SegregationStatus;
use App\Repositories\SegregationStatusRepository;

class SegregationStatusController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        if(Auth::user()->hasPermission($this->route->module,$this->route->action)) {
            $SegregationStatusRepository = new SegregationStatusRepository();
            $parameters = array();
            ...
            $parameters['list'] = $SegregationStatusRepository->findBy($parameters, ['id ASC'], 20, 10);
     ...
     *
     * @param  \Illuminate\Http\Request  $request
     * @param  \Integer $segregation_status_id
     * @return \Illuminate\Http\Response
     */
    public function edit(Request $request, $segregation_status_id)
    {        
        if(Auth::user()->hasPermission($this->route->module,$this->route->action)) {
            $parameters = array();
            $parameters = $request->except('_token');
            $parameters['segregation_status_id'] = (isset($parameters['segregation_status_id'])) ? ($parameters['segregation_status_id']) : $segregation_status_id; 
            $parameters['name'] = (isset($parameters['name'])) ? ($parameters['name']) : ""; 
            $parameters['route'] = $this->route;
            if(!empty($segregation_status_id)) {                                                               
                $SegregationStatusRepository = new SegregationStatusRepository();
                $SegregationStatus = $SegregationStatusRepository->findById($segregation_status_id);
                foreach($SegregationStatus->toArray() as $key => $value) {
     ...
     * @return \Illuminate\Http\Response
     */
    public function delete(Request $request, $segregation_status_id)
    {        
        $SegregationStatusRepository = new SegregationStatusRepository();        
        if(!empty($segregation_status_id)) {
            $SegregationStatus = $SegregationStatusRepository->findById($segregation_status_id);
            $SegregationStatus->active = false;
            $SegregationStatusRepository->save($SegregationStatus);
        }
        //Response
        $parameters = array();
    ...
    public function save(Request $request)
    {
        ...
        switch($procedure) {
            case 'create':
                $SegregationStatus = new SegregationStatus();
                $SegregationStatusRepository = new SegregationStatusRepository();
                $SegregationStatus->name = (!empty($name)) ? $name : "";
                $SegregationStatus = $SegregationStatusRepository->save($SegregationStatus);
                //Response
                $parameters = array();
