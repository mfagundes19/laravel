...
class RegionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $parameters['list'] = $RegionRepository->findBy($parameters, ['id ASC'], 20, 10);
     ...
     * @return \Illuminate\Http\Response
     */
    public function edit(Request $request, $region_id = null)
    {        
        if(Auth::user()->hasPermission($this->route->module,$this->route->action)) {
            $parameters = array();
            $parameters = $request->except('_token');
            $parameters['region_id'] = (isset($parameters['region_id'])) ? ($parameters['region_id']) : $region_id; 
            $parameters['name'] = (isset($parameters['name'])) ? ($parameters['name']) : ""; 
            $parameters['route'] = $this->route;
            if(isset($region_id) && !empty($region_id)) {                               
                $RegionRepository = new RegionRepository();
                $Region = $RegionRepository->findById($region_id);
     ...
     * @return \Illuminate\Http\Response
     */
    public function delete(Request $request, $region_id)
    {        
        if(Auth::user()->hasPermission($this->route->module, $this->route->action)) {  
            $RegionRepository = new RegionRepository();
            if(isset($region_id) && !empty($region_id)) {
