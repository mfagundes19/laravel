...
class OperationTypeController extends Controller
{
    ...
    public function index(Request $request)
    {
        if(Auth::user()->hasPermission($this->route->module, $this->route->action)) { 
            ...
            $parameters['list'] = $OperationTypeRepository->findBy($parameters, ['id ASC'], 20, 10);
     ...
     *
     * @param  \Illuminate\Http\Request  $request
     * @param  \Integer $operation_type_id
     * @return \Illuminate\Http\Response
     */
    public function edit(Request $request, $operation_type_id)
    {        
        if(Auth::user()->hasPermission($this->route->module,$this->route->action)) {
            $parameters = array();
            $parameters = $request->except('_token');
            $parameters['operation_type_id'] = (isset($parameters['operation_type_id'])) ? ($parameters['operation_type_id']) : $operation_type_id; 
            $parameters['name'] = (isset($parameters['name'])) ? ($parameters['name']) : ""; 
            $parameters['route'] = $this->route;
            if(isset($operation_type_id) && !empty($operation_type_id)) {
                $OperationTypeRepository = new OperationTypeRepository();
                $OperationType = $OperationTypeRepository->findById($operation_type_id);
                foreach($OperationType->toArray() as $key => $value) {
     ...
     * @return \Illuminate\Http\Response
     */
    public function delete(Request $request, $operation_type_id)
    {        
        if(Auth::user()->hasPermission($this->route->module, $this->route->action)) {
            $OperationTypeRepository = new OperationTypeRepository();
            if(isset($operation_type_id) && !empty($operation_type_id)) {
                $OperationType = $OperationTypeRepository->findById($operation_type_id);
                $OperationType->active = false;
    ...
    public function save(Request $request)
    {
        ...
        switch($procedure) {
            case 'create':   //Create
                ...
                $Repository = new OperationTypeRepository();
                $OperationType->name = (!empty($name)) ? $name : "";
                $OperationType = $Repository->save($OperationType);
                //Response
                $parameters = array();
                $parameters['message'] = $this->getMessage('creted');
                $parameters['route'] = $this->route;
