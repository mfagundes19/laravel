...
<div class="content">
    <div class="row row-content-page">
        ...
        <div class="col-lg-12">
            {!! Form::open(['url' => url($route->module.'/edit'),'id' => 'form-'.$route->module,'method' => 'POST','data-action' => url($route->module.'/save'),'data-view' => $route->module.'/edit']) !!}  
                        ...
                        <table class="table table-bordered table-list">
                            ...
                            <tbody>
                                @foreach (\App\Models\Module::all() as $key => $element)
                                    @php                            
                                    $module_view[$key] = (isset($module_view[$key]) && $module_view[$key] == true) ? true : false;
                                    $module_create[$key] = (isset($module_create[$key]) && $module_create[$key] == true) ? true : false;
                                    $module_edit[$key] = (isset($module_edit[$key]) && $module_edit[$key] == true) ? true : false;
                                    $module_delete[$key] = (isset($module_delete[$key]) && $module_delete[$key] == true) ? true : false;
                                    $module_extra[$key] = (isset($module_extra[$key]) && $module_extra[$key] == true) ? true : false;
                                    @endphp
                                    <tr>
                                        <td>
                                            {{$element->name}}
